				...
				<c:if test="${ !empty list }">
					<div style="text-align: center; width:1500px; margin-top:10px;">
						...
						<c:if test="${ currentPage gt 1 }">
							<a
								href="${ urlMapping }?page=1&studentCode=${ sessionScope.student.studentCode }">[맨처음]</a> &nbsp;
   						</c:if>

						<c:if
							test="${ (currentPage - 10) lt startPage and (currentPage - 10) gt 1 }">
							<a
								href="${ urlMapping }?page=${ startPage - 10 }&studentCode=${ sessionScope.student.studentCode }">[이전그룹]</a> &nbsp;
   						</c:if>
						...
						<c:forEach var="p" begin="${ startPage }" end="${ endPage }"
							...
							<c:if test="${ p ne currentPage }">
								<a
									href="${ urlMapping }?page=${ p }&studentCode=${ sessionScope.student.studentCode }">${ p }</a>
							</c:if>
						</c:forEach>
						...
						<c:if
							...
							<a
								href="${ urlMapping }?page=${ startPage + 10 }&studentCode=${ sessionScope.student.studentCode }">[다음그룹]</a> &nbsp;
   						</c:if>
						...
						<c:if test="${ currentPage < maxPage }">
							<a
								href="${ urlMapping }?page=${ maxPage }&studentCode=${ sessionScope.student.studentCode }">[맨끝]</a> &nbsp;
   						</c:if>
					</div>
				</c:if>
